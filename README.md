...
| 黄金 OHLC / 美元指数 | 美股 17:00 ET 收盘后 | 增量缓存, 目标日 = 最近已收盘美股交易日, 收盘前不抓当天 |
| 沪金期权 PCR | 当日晚间 (北京时间) | 增量 + gaps 缓存, 只补到昨天 (今天的文件晚间才发布) |
| 美国 GLD 期权 PCR | 美股收盘后 | 快照按**美股交易日**打戳 (非运行日), 缓存已有该日则跳过 |

GitHub Actions 每天北京时间 07:30 运行一次: 此时沪金昨日数据已发布、
美股前一日已收盘, 周六运行可抓到周五发布的 COT——四个源全部命中发布时点。
